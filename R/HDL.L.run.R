...
source(paste0(save.path,"HDL.L.R"))

## default arguments
Nref <- ifelse(length(Nref)==0, 335272, as.numeric(Nref))
N0 <- ifelse(length(N0)==0, 0, as.numeric(N0))
eigen.cut <- ifelse(length(eigen.cut)==0, 0.95, as.numeric(eigen.cut))
...
process_function <- function(chr, piece) {
  ...
  tryCatch({
    res <- HDL.L(gwas1.df, gwas2.df, Trait1name = Trait1name, Trait2name = Trait2name, 
                 ...
                 output.file = "", N0 = N0, lim = lim, Nref = Nref, eigen.cut = eigen.cut)
    return(res)
  }, error = function(e) {
...
if(type == "WG"){
    results <- mcmapply(process_function, as.numeric(nsnps.df.imputed$chr), as.numeric(nsnps.df.imputed$piece), SIMPLIFY = FALSE, mc.cores = cores)
...
res.HDL <- do.call(rbind, results)
cat("Saving results ... \n")
save(res.HDL, file = paste0(save.path,"res_HDL",Trait1name,"_", Trait2name,".RData"))
