...
#' @examples
#'\dontrun{
#' ## The GWAS summary statistics for basal metabolic rate
#' data(gwas1.example)
#'
#' ## The GWAS summary statistics for standing height
#' data(gwas2.example)
#'
#' ## # The path to the directory where linkage disequilibrium (LD) and bim files are stored.
...
#' }
#' @import dplyr
#' @importFrom stats cor lm median optim pchisq qchisq
...
if(intercept.output == FALSE){
      estimates.df <- matrix(c(h11,p.h1.lrt,h22,p.h2.lrt, h12, rg, rg.lower, rg.upper, p.h12.lrt), nrow=1,ncol=9)
      colnames(estimates.df) <- c("Heritability_1","P_value_Heritability_1","Heritability_2", "P_value_Heritability_2", "Genetic_Covariance","Genetic_Correlation","Lower_bound_rg","Upper_bound_rg", "P")
    } else{
      estimates.df <- matrix(c(h11,p.h1.lrt,h22,p.h2.lrt, h12, rg, rg.lower, rg.upper, p.h12.lrt, h11.intercept, NA, h22.intercept, NA, h12.intercept,NA,NA,NA,NA), nrow=2,ncol=9)
      colnames(estimates.df) <- c("Heritability_1","P_value_Heritability_1", "Heritability_2", "P_value_Heritability_2", "Genetic_Covariance","Genetic_Correlation","Lower_bound_rg","Upper_bound_rg", "P")
